    ...
    <table class="ui celled table" v-if="!isAtomicSwap">
      ...
      <tbody>
        ...
        <tr v-if="isBlockCreatorReward">
          ...
          <td
            class="clickable"
            v-on:click="routeToBlockPage(coinOutput.blockId)"
          >
            {{ coinOutput.blockId }}
          </td>
        </tr>
        ...
        <tr v-if="isLegacy">
          ...
          <td
            class="clickable"
            v-on:click="routeToHashPage(coinOutput.unlockhash)"
          >
        ...
        <tr v-else>
          ...
          <td
            class="clickable"
            v-on:click="routeToHashPage(coinOutput.condition.data.unlockhash)"
          >
        ...
        <tr v-if="isLegacy">
          <td>Value</td>
          <td>
            {{ toLocalDecimalNotation(coinOutput.value / precision) }}
            {{ unit }}
          </td>
        </tr>
        <tr v-else>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td
            ...
            v-on:click="
              routeToHashPage(
                $store.getters.HASH.transactions[coinOutput.transactionsIdx]
                  .coinoutputunlockhashes[coinOutput.coinOutputIdx]
              )
            "
          >
            {{
        ...
        <tr>
          ...
          <td
            class="clickable"
            v-on:click="routeToHashPage(coinOutput.condition.data.sender)"
          >
        ...
        <tr>
          ...
          <td
            class="clickable"
            v-on:click="routeToHashPage(coinOutput.condition.data.receiver)"
          >
...
export default class CoinOutputHash extends Vue {
  ...
  getCoinOutput() {
    ...
    let transactionsIndex = coinOutputIndexArray.findIndex(
      (idx: any) => idx !== -1
    );
    ...
    if (!transactions[transactionsIndex].rawtransaction.data.coinoutputs)
      return;
    const coinoutput =
      transactions[transactionsIndex].rawtransaction.data.coinoutputs[
        coinOutputIndex
      ];
  ...
  getCoinOutputBlockCreatorReward() {
    ...
    let transactionsIndex = minerPayoutsIndexArray.findIndex(
      (idx: any) => idx !== -1
    );
    // if index is -1, no output is found. Return nothing
    if (transactionsIndex == -1) return;
    ...
    const coinoutput =
      blocks[transactionsIndex].rawblock.minerpayouts[coinOutputIndex];
  ...
  getCoinInput() {
    ...
    let coinInputIndexArray = transactions.map((tx: any) => {
      ...
      return tx.rawtransaction.data.coininputs.findIndex(
        (ci: any) => ci.parentid === hashId
      );
    });
    let transactionsIndex = coinInputIndexArray.findIndex(
      (idx: any) => idx !== -1
    );
    ...
    const coininput =
      transactions[transactionsIndex].rawtransaction.data.coininputs[
        coinInputIndex
      ];
