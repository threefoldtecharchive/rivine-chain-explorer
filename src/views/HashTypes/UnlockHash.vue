<template lang="html">
  <div class="container">
    ...
    <div>
        ...
        <tbody>
          ...
          <tr v-if="!isAtomicSwap && lastCoinSpent">
            ...
            <td>
              ...
              <span
                class="clickable"
                v-on:click="routeToBlockPage(lastCoinSpent.height)"
                >{{ lastCoinSpent.height }}</span
              >
              Txid:
              <span
                class="clickable"
                v-on:click="routeToHashPage(lastCoinSpent.txid)"
                >{{ lastCoinSpent.txid }}</span
              >
            </td>
          </tr>
          <tr>
          ...
          <tr v-if="this.$store.getters.HASH.blocks && lastBsSpent">
            ...
            <td>
              ...
              <span
                class="clickable"
                v-on:click="routeToBlockPage(lastBsSpent.height)"
                >{{ lastBsSpent.height }}</span
              >
              Txid:
              <span
                class="clickable"
                v-on:click="routeToHashPage(lastBsSpent.txid)"
                >{{ lastBsSpent.txid }}</span
              >
            </td>
          </tr>
        </tbody>
      </table>
      <div v-for="(uco, idx) in ucos" v-bind:key="idx">
          ...
          <table class="ui celled table">
            <tbody>
              ...
              <tr v-if="uco.condition">
                ...
                <td
                  class="clickable"
                  v-on:click="routeToHashPage(uco.condition.data.unlockhash)"
                >
                  {{ uco.condition.data.unlockhash }}
                </td>
              </tr>
              <tr v-else>
                <td>Address</td>
                <td
                  class="clickable"
                  v-on:click="routeToHashPage(uco.unlockhash)"
                >
                  {{ uco.unlockhash }}
                </td>
              </tr>
      ...
      <div v-for="(sco, idx) in scos" v-bind:key="idx">
        <div class="tx-table">
          <table class="ui celled table tx-table">
            <tbody>
              <tr>
                ...
                <td
                  class="clickable"
                  v-on:click="routeToBlockPage(sco.blockHeight)"
                >
                  {{ sco.blockHeight }}
                </td>
              </tr>
              <tr>
                <td>Transaction ID</td>
                <td class="clickable" v-on:click="routeToHashPage(sco.txid)">
                  {{ sco.txid }}
                </td>
              </tr>
              <tr>
                ...
                <td
                  class="clickable"
                  v-on:click="routeToHashPage(sco.coinOutputId)"
                >
                  {{ sco.coinOutputId }}
                </td>
              </tr>
              <tr v-if="sco.condition">
                ...
                <td
                  class="clickable"
                  v-on:click="routeToHashPage(sco.condition.data.unlockhash)"
                >
                  {{ sco.condition.data.unlockhash }}
                </td>
              </tr>
              <tr v-else>
                <td>Address</td>
                <td
                  class="clickable"
                  v-on:click="routeToHashPage(sco.unlockhash)"
                >
                  {{ sco.unlockhash }}
                </td>
              </tr>
              <tr>
                <td>Value</td>
                <td>
                  {{ toLocalDecimalNotation(sco.value / precision) }} {{ unit }}
                </td>
              </tr>
              <tr>
                <td>Has Been Spent</td>
      ...
      <h3 v-if="unspentMinerPayouts.length > 0">
        Fee/Reward Payout Appearances
      </h3>
      <div v-for="(unspentMp, idx) in unspentMinerPayouts" v-bind:key="idx">
        <div class="tx-table">
          <table class="ui celled table tx-table">
            <tbody>
              <tr>
                ...
                <td
                  class="clickable"
                  v-on:click="routeToBlockPage(unspentMp.blockHeight)"
                >
                  {{ unspentMp.blockid }}
                </td>
              </tr>
              <tr>
                ...
                <td
                  class="clickable"
                  v-on:click="routeToHashPage(unspentMp.minerPayoutId)"
                >
                  {{ unspentMp.minerPayoutId }}
                </td>
              </tr>
              <tr>
              ...
              <tr>
                <td>Value</td>
                <td>
                  {{ toLocalDecimalNotation(unspentMp.value / precision) }}
                  {{ unit }}
                </td>
              </tr>
              <tr>
      ...
      <h3 v-if="spentCoinOutputsBlockCreator.length > 0">
        Coin Output Appearances
      </h3>
      <div v-for="(sco, idx) in spentCoinOutputsBlockCreator" v-bind:key="idx">
        <div class="tx-table">
          <table class="ui celled table tx-table">
            <tbody>
              <tr>
      ...
      <h3 v-if="spentBlockStakesOutputsBlockCreator.length > 0">
        Blockstake Output Appearances
      </h3>
      <div
        v-for="(sbo, idx) in spentBlockStakesOutputsBlockCreator"
        v-bind:key="idx"
      >
        <div class="tx-table">
          <table class="ui celled table tx-table">
            <tbody>
              <tr>
                ...
                <td
                  class="clickable"
                  v-on:click="routeToBlockPage(sbo.blockHeight)"
                >
                  {{ sbo.blockHeight }}
                </td>
              </tr>
              <tr>
                <td>Transaction ID</td>
                <td class="clickable" v-on:click="routeToHashPage(sbo.txid)">
                  {{ sbo.txid }}
                </td>
              </tr>
              <tr>
                ...
                <td
                  class="clickable"
                  v-on:click="routeToHashPage(sbo.blockstakeOutputId)"
                >
                  {{ sbo.blockstakeOutputId }}
                </td>
              </tr>
              <tr>
                <td>Address</td>
      ...
      <h3 v-if="unspentBlockStakesOutputsBlockCreator.length > 0">
        Blockstake Output Appearances
      </h3>
      <div
        v-for="(usbo, idx) in unspentBlockStakesOutputsBlockCreator"
        v-bind:key="idx"
      >
        <div class="tx-table">
          <table class="ui celled table tx-table">
            <tbody>
              <tr>
                ...
                <td
                  class="clickable"
                  v-on:click="routeToBlockPage(usbo.blockHeight)"
                >
                  {{ usbo.blockHeight }}
                </td>
              </tr>
              <tr>
                <td>Transaction ID</td>
                <td class="clickable" v-on:click="routeToHashPage(usbo.txid)">
                  {{ usbo.txid }}
                </td>
              </tr>
              <tr>
                ...
                <td
                  class="clickable"
                  v-on:click="routeToHashPage(usbo.blockstakeOutputId)"
                >
                  {{ usbo.blockstakeOutputId }}
                </td>
              </tr>
              <tr>
                <td>Address</td>
