  ...
  <div class="container">
      ...
      <tbody>
        <tr>
          ...
          <td
            ...
            class="clickable"
            v-on:click="
              routeToBlockPage($store.getters.HASH.transaction.height)
            "
          >
            {{
              toLocalDecimalNotation(
                this.$store.getters.HASH.transaction.height
              )
            }}
          </td>
        </tr>
        <tr v-if="this.$store.getters.HASH.transaction.height !== 0">
          ...
          <td>
            {{
              this.$store.getters.EXPLORER.height -
                this.$store.getters.HASH.transaction.height +
                1
            }}
          </td>
    ...
    <div v-if="blockStakeInputs.length > 0" class="tx-table">
      ...
      <table
        ...
        v-for="(bsInput, index) in blockStakeInputs"
        v-bind:key="index"
      >
        <thead>
          <tr>
        ...
        <tbody>
          <tr>
            ...
            <td
              class="clickable"
              v-on:click="routeToHashPage(bsInput.parentid)"
            >
              {{ bsInput.parentid }}
            </td>
          </tr>
    ...
    <div v-if="blockStakeOutputs.length > 0" class="tx-table">
      ...
      <table
        v-for="(bsOut, index) in blockStakeOutputs"
        class="ui celled table"
        v-bind:key="index"
      >
        <tbody>
          ...
          <tr v-if="isLegacy">
            ...
            <td
              class="clickable"
              v-on:click="routeToHashPage(bsOut.unlockhash)"
            >
              {{ bsOut.unlockhash }}
            </td>
          </tr>
    ...
    <div v-if="coinInputs.length > 0" class="tx-table">
      ...
      <table
        class="ui celled table"
        v-for="(coinInput, index) in coinInputs"
        v-bind:key="index"
      >
        <thead>
          <tr>
        ...
        <tbody>
          <tr>
            ...
            <td
              class="clickable"
              v-on:click="routeToHashPage(coinInput.parentid)"
            >
              {{ coinInput.parentid }}
            </td>
          </tr>
    ...
    <div v-if="coinOutputs.length > 0" class="tx-table">
      ...
      <div v-for="(coinOut, index) in coinOutputs" v-bind:key="index">
        <!-- if legacy or condition type 1 -->
        <table
          class="ui celled table"
          v-if="coinOut.value || coinOut.condition.type === 1"
        >
          <tbody>
            <tr>
              <td>ID</td>
            ...
            <tr v-if="isLegacy">
              ...
              <td
                class="clickable"
                v-on:click="routeToHashPage(coinOut.unlockhash)"
              >
                {{ coinOut.unlockhash }}
              </td>
            </tr>
        ...
        <table
          class="ui celled table"
          v-else-if="coinOut.condition && coinOut.condition.type === 2"
        >
          <thead>
            <tr>
          ...
          <tbody>
            ...
            <tr v-if="output.condition">
              ...
              <td
                ...
                v-on:click="
                  routeToHashPage(
                    $store.getters.HASH.transaction.coinoutputunlockhashes[
                      index
                    ]
                  )
                "
            ...
            <tr v-if="output.condition.data.sender">
              ...
              <td
                class="clickable"
                v-on:click="routeToHashPage(output.condition.data.sender)"
              >
            ...
            <tr v-if="output.condition.data.receiver">
              ...
              <td
                class="clickable"
                v-on:click="routeToHashPage(output.condition.data.receiver)"
              >
            ...
            <tr>
              <td>Value</td>
              <td>
                {{ toLocalDecimalNotation(output.value / precision) }}
                {{ unit }}
              </td>
            </tr>
          </tbody>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              {{
                this.$store.getters.HASH.transaction.rawtransaction.data
                  .mintfulfillment.data.publickey
              }}
            </td>
          </tr>
          <tr>
            ...
            <td>
              {{
                this.$store.getters.HASH.transaction.rawtransaction.data
                  .mintfulfillment.data.signature
              }}
            </td>
          </tr>
        </tbody>
        ...
        <tbody>
          <tr>
            ...
            <td
              ...
              v-on:click="
                routeToBlockPage(
                  $store.getters.HASH.transaction.rawtransaction.data
                    .mintcondition.data.unlockhash
                )
              "
            >
              {{
    ...
    >
      ...
      <table class="ui celled table">
        <tbody>
          <tr>
            <td>ID</td>
            <td class="clickable" v-on:click="routeToHashPage(feepayoutId)">
              {{ feepayoutId }}
            </td>
          </tr>
          <tr>
            ...
            <td
              class="clickable"
              v-on:click="routeToHashPage(feepayoutAddress)"
            >
              {{ feepayoutAddress }}
            </td>
          </tr>
          <tr>
            <td>Value</td>
            <td>
              {{ toLocalDecimalNotation(txFee / precision) }} {{ unit }} of a
              total payout of
              {{ toLocalDecimalNotation(feepayoutValue / precision) }}
              {{ unit }}
            </td>
          </tr>
        </tbody>
...
export default class TransactionIdHash extends Vue {
  ...
  getTransactionType() {
    ...
    switch (this.$store.getters.HASH.transaction.rawtransaction.version) {
      case 128:
        this.version = "Condition Update Transaction";
        break;
      case 129:
        this.version = "Coin Creation Transaction";
        break;
      case 130:
        this.version = "Coin Destruction Transaction";
        break;
    }
  }
  ...
  fetchExplorerBlock() {
    ...
    axios({
      method: "GET",
      url: API_URL + "/explorer/hashes/" + parentId
    }).then(result => {
      const explorerBlock = result.data.block;
      if (
        explorerBlock.rawblock.minerpayouts == null ||
        explorerBlock.rawblock.minerpayouts.length < 2
      ) {
        return null;
      }
      ...
      if (this.$store.getters.HASH.transaction.rawtransaction.data.minerfees) {
        let sumFee = 0;
        this.$store.getters.HASH.transaction.rawtransaction.data.minerfees.map(
          (fee: number) => {
            sumFee += fee;
          }
        );
